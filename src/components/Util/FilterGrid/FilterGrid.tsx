...
class FilterGrid extends Component<FilterGridProps, FilterGridState> {
	...
	render() {
		...
		const FILTER_BUTTONS: JSX.Element[] = this.props.tags.map((filter: Filter) => (
			<FilterToggle
		...
		const numberOfFilters: number = this.state.showMore ? FILTER_BUTTONS.length : DEFAULT_NUMBER_OF_FILTERS;
